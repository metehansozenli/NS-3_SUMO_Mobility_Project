...
LoraPacketTracker::CountMacPacketsGlobally(Time startTime, Time stopTime)
{
    ...
    double sent = 0;
    double received = 0;
    double totalDelay = 0; // Toplam gecikme süresini hesaplamak için
    ...
    for (auto it = m_macPacketTracker.begin(); it != m_macPacketTracker.end(); ++it)
    {
        if ((*it).second.sendTime >= startTime && (*it).second.sendTime <= stopTime)
        {
            ...
            {
                // İlk alım zamanını elde ediyoruz
                Time firstReceivedTime = (*it).second.receptionTimes.begin()->second;
                
                // İletim zamanını ve alım zamanını saniye cinsinden alıyoruz
                double sendTimeSeconds = (*it).second.sendTime.GetSeconds();
                double receivedTimeSeconds = firstReceivedTime.GetSeconds();
                
                // Gecikme süresini hesaplıyoruz
                double delay = receivedTimeSeconds - sendTimeSeconds;
                
                // Toplam gecikme süresini güncelliyoruz
                totalDelay += delay;
    ...
    // Ortalama gecikme süresini hesaplıyoruz
    double averageDelay = (received > 0) ? totalDelay / received : 0.0;
    double PDR = (sent > 0) ? (received / sent) * 100 : 0.0; 
    return std::to_string(sent) + " " + std::to_string(received) + " " + std::to_string(PDR) + "% " +           std::to_string(averageDelay) + "s";
}
